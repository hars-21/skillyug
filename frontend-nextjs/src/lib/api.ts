...
export const API_ENDPOINTS = {
  ...
  // Payments - using full backend URL
  PAYMENTS: {
    GET_KEY: `${BACKEND_URL}/api/getKey`,
    CHECKOUT: `${BACKEND_URL}/api/checkout`,
    VERIFY: `${BACKEND_URL}/api/paymentverification`,
  },
...
export const apiHelpers = {
  ...
  get: async <T = unknown>(endpoint: string, params?: Record<string, string | number | boolean>): Promise<T> => {
    try {
  ...
  post: async <T = unknown, D = Record<string, unknown>>(endpoint: string, data?: D): Promise<T> => {
    try {
  ...
  delete: async <T = unknown>(endpoint: string, params?: Record<string, string | number | boolean>): Promise<T> => {
    try {
...
export const paymentAPI = {
  getRazorpayKey: async (): Promise<{ key: string }> => {
    const response = await axios.get(API_ENDPOINTS.PAYMENTS.GET_KEY);
    return response.data;
  },

  createOrder: async (amount: number, courseId: string) => {
    const response = await axios.post(API_ENDPOINTS.PAYMENTS.CHECKOUT, {
      amount,
      courseId,
    });
    return response.data;
  ...
  verifyPayment: async (paymentData: {
    ...
    const response = await axios.post(API_ENDPOINTS.PAYMENTS.VERIFY, paymentData);
    return response.data;
  },
};
